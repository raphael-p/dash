...
func fillTasksWithSampleData() error {
	...
	_, err := DB.Exec(insertTask, "datajack override", "bypass corporate firewalls and extract encrypted payloads", createdTime0, updatedTime0)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "neon trace", "track digital footprints across the city's undernet", createdTime1, updatedTime1)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "synthwave uplink", "Establish a covert connection to a rogue satellite", createdTime2, updatedTime2)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "chrome smuggler", "transport illegal cyberware through police scanners", createdTime3, updatedTime3)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "ghostline breach", "penetrate abandoned server farms for lost archives", createdTime4, updatedTime4)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "black ICE hunter", "neutralize hostile AI defenses in corporate systems", createdTime5, updatedTime5)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "memory forge", "fabricate false identity packets for underground networks", createdTime6, updatedTime6)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "holojack hijack", "seize control of public hologram billboards for rebel propaganda", createdTime7, updatedTime7)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "byte runner", "deliver volatile data drives through surveillance zones", createdTime8, updatedTime8)
	if err != nil {
		return err
	}
	...
	_, err = DB.Exec(insertTask, "circuit prophet", "decode predictive algorithms from seized tech cult servers", createdTime9, updatedTime9)
	if err != nil {
		return err
	}
